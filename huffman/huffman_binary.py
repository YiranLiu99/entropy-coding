...
def write_compressed_data(compressed_data, save_dir_path, file_name):
    save_file_full_name = 'huf_compressed_' + file_name + '.bin'
...
def write_decompressed_data(decompressed_data, save_dir_path, file_name, file_extension):
    save_file_full_name = 'huf_decompressed_' + file_name + file_extension
    save_file_path = save_dir_path + '/' + save_file_full_name
    with open(save_file_path, 'wb') as file:
...
def to_decompressed_data(compressed_data_path):
    with open(compressed_data_path, 'rb') as file:
        ...
        try:
            ...
            raise Exception('Invalid compressed huffman data.')
        if compress_version != 'huf_bin':
            raise Exception('Invalid compressed huffman data.')
...
class HuffmanBinary:
    ...
    def process_data(self):
        (self.frequencies,
         ...
         self.root_node_id) = huffman_coding.compress_data(self.data)
    ...
    def to_compressed_data(self):
        ...
        compressed_data = {
            ...
            'compress_version': 'huf_bin',  # (str type)
...
if __name__ == '__main__':
    ...
    save_compressed_file_path = save_dir_path + '/huf_compressed_' + file_name + '.bin'
    # save_decompressed_file_path = save_dir_path + '/huf_decompressed_' + file_name + file_extension

    start_time = time.time()
    print('Compressing the file ' + file_path)
    huff_binary = HuffmanBinary(file_path)
    huff_binary.process_data()
    compressed_data_bytes = huff_binary.to_compressed_data()
    write_compressed_data(compressed_data_bytes, save_dir_path, huff_binary.file_name)
